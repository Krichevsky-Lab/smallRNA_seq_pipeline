...
process_one_bam <- function(bam_path, sample_id) {
  ...
  if (file.exists(out_file)) {
    message("⏩ Skipping sample ", sample_id)
  ...
  message("💾 Saved per-sample tRFs → ", out_file)
  df_final
}
...
message("✅ Saved final combined tRF table → ", out_csv)
message("📦 Samples included: ", length(sample_cols))
message("📊 Total tRF entries: ", nrow(out_df))
