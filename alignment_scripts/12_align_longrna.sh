...
# ------------------------------------------------------------------------------
# 12_align_longrna.sh
# Align piRNA-rescue–unmapped reads to long RNA reference using Bowtie1.
#
# Usage:
#   bash alignment_scripts/12_align_longrna.sh --species <mouse|human> [--qc]
#   SPECIES=mouse bash alignment_scripts/12_align_longrna.sh [--qc]
#
# Inputs:
...
INPUT_DIR="alignment_output/unmapped/pirna_bowtie2"
ALIGN_DIR="alignment_output/aligned/longrna_bowtie1"
UNMAP_DIR="alignment_output/unmapped/longrna_bowtie1"
COUNTS_DIR="alignment_output/counts/longrna_bowtie1"
LOG_DIR="alignment_output/log/longrna_bowtie1"
QC_DIR="alignment_output/qc/longrna_bowtie1"
...
step_banner 12 "longRNA alignment (Bowtie1)"
...
for fq in "${inputs[@]}"; do
  ...
  if ! check_bam "$bam_out"; then
    ...
    if ! check_bam "$bam_out"; then
      log_warn "$sample — no alignments, skipping"
      continue
    fi

    samtools index "$bam_out"
    log_done "$sample — BAM ready"
  else
...
Rscript "$SCRIPT_DIR/combine_counts.R" \
  "$COUNTS_DIR" "_longrna_counts\\.txt$" "_longrna_counts.txt" \
...
log_done "Step 12 — longRNA Bowtie1 alignment complete"
exit $rc_all
