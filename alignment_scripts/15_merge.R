...
#
# Usage:
#   Rscript alignment_scripts/14_merge.R <mouse|human>
#   SPECIES=mouse Rscript alignment_scripts/14_merge.R
#
# Outputs:
#   analysis/<species>/rna_counts.csv
#   analysis/<species>/mapped_reads_by_biotype_2.csv
#   analysis/<species>/percentage_mapped_by_biotype_2.csv
#
################################################################################
...
SPECIES <- if (length(args) >= 1) args[1] else Sys.getenv("SPECIES")

if (SPECIES == "") stop("Species required. Pass as arg or set $SPECIES env var.")
if (!SPECIES %in% c("mouse", "human")) stop(paste("Unknown species:", SPECIES))

message("[INFO]  Species: ", SPECIES)

# ================================
# Paths
# ================================
ROOT_DIR     <- getwd()
COUNTS_DIR   <- file.path(ROOT_DIR, "alignment_output", "counts")
DATA_DIR     <- file.path(ROOT_DIR, "analysis", "data")
LOG_FILE     <- file.path(ROOT_DIR, "alignment_output", "log", "rrna_bowtie1", "read_counts.csv")
RUNTIME_FILE <- file.path(ROOT_DIR, "alignment_output", "log", "pipeline_runtime.csv")

dir_create(DATA_DIR)

OUT_COUNTS   <- file.path(DATA_DIR, "rna_counts.csv")
OUT_READS    <- file.path(DATA_DIR, "alignment_stats.csv")
OUT_PCT      <- file.path(DATA_DIR, "alignment_pct.csv")

message("[INFO]  Pipeline root: ", ROOT_DIR)

# ================================
# Safety checks
# ================================
if (!dir_exists(COUNTS_DIR)) stop("Missing alignment_output/counts/")
...
merge_two <- function(f1, f2) {
  ...
  for (s in sample_cols) {
    ...
    has_c1 <- c1 %in% names(merged)
    has_c2 <- c2 %in% names(merged)
    if (has_c1 || has_c2) {
      v1 <- if (has_c1) ifelse(is.na(merged[[c1]]), 0, merged[[c1]]) else 0
      v2 <- if (has_c2) ifelse(is.na(merged[[c2]]), 0, merged[[c2]]) else 0
      merged[[s]] <- v1 + v2
      if (has_c1) merged[[c1]] <- NULL
      if (has_c2) merged[[c2]] <- NULL
    }
    # else: s exists unsuffixed (only in one file) — leave as-is
  }
...
message("[DONE]  rna_counts.csv written")
...
message("[DONE]  mapped_reads_by_biotype_2.csv written")
...
message("[DONE]  alignment_pct.csv written")

# ================================
# Pipeline runtime summary
# ================================
if (file.exists(RUNTIME_FILE)) {
  runtime <- read_csv(RUNTIME_FILE, show_col_types = FALSE) %>%
    mutate(elapsed_min = round(elapsed_seconds / 60, 2)) %>%
    select(step, script, status, elapsed_seconds, elapsed_min, timestamp)
  write_csv(runtime, file.path(DATA_DIR, "pipeline_runtime.csv"))
  message("[DONE]  pipeline_runtime.csv written")
} else {
  message("[WARN]  pipeline_runtime.csv not found — skipping runtime summary")
}
